...
int ttydiscipline(char ch,
	...
	if (ch == TY_NEWLINE || ch == TY_RETURN) {
		 ...
		 */
		char* tyihead_cpy = typtr->tyihead;
		strcpy(typtr->typrev, "");
		int i = 0;
		while(typtr->tyihead != typtr->tyitail && i < 127) {
		
			strcpy(typtr->typrev + i++, typtr->tyihead++);
			/* Wrap around buffer, if needed */
			if (typtr->tyihead >= &typtr->tyebuff[TY_EBUFLEN])
				typtr->tyehead = typtr->tyebuff;
		}
		char null_ch = '\0';
		strcpy(typtr->typrev + i, &null_ch);

		typtr->tyihead = tyihead_cpy;
	}
